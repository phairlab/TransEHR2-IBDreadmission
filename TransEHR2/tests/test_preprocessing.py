"""One test per claim the extraction path makes.
...
def test_bucket_returns_four_buckets_with_text_and_drug_together():
    """The buckets are numeric / categorical / ordinal /
...
def test_single_slot_lookup_writes_no_dose_or_mask_array(one_patient):
    """There are drug_doses and drug_masks and no text
...
def test_text_values_are_rank_one_drug_values_are_slotted(one_patient):
    """text_values is (n_nonempty,), drug_values is
    (n_nonempty, n_slots)."""
    assert run(one_patient) == 0
...
def test_out_of_domain_value_is_minus_one_and_counted(mini, capsys):
    """Index -1 means 'observed, but not in category_map'.
...
def test_unobserved_is_indicator_zero_and_index_minus_one(one_patient):
    """The other half of the code: (0, -1) is 'not observed'. The arrays are
...
def test_numeric_looking_string_levels_map(one_patient):
    """The dtype trap: ORD's levels are the strings '0',
...
def test_filter_keeps_the_last_records_not_the_first():
    """This is the opposite end of the window from the one the filter
    was originally written for, which kept the first N."""
...
def test_filter_includes_a_record_exactly_at_the_index_time():
    """Every INDEX_TIME names a timeseries.csv row, and t = 0 sits on
    the last record."""
...
def test_times_are_int32_minutes_before_the_index_time(one_patient):
    """Integer minutes, <= 0, int32. Not hours or days --
...
def test_series_is_right_aligned_and_zero_padded_on_the_left(one_patient):
    """The prediction origin lands in the final column of
...
def test_no_empty_bins_between_distant_records(mini):
    """``.resample('1h')`` materializes every hourly bin
...
def test_drug_slots_are_padded_with_the_vocabulary_row_count(one_patient):
    """Unused slots take V, the pad row of a (V+1, 128)
...
def test_drug_masks_agree_with_the_pad_index(one_patient):
    """drug_masks is kept even though it is derivable from
...
def test_a_dispensation_with_no_valued_feature_still_gets_a_timestep(
    ...
    """Such a row is carried by the registry alone, and every drugs.csv
    timestamp is required to name a timeseries.csv row."""
    assert run(one_patient) == 0
...
def test_a_drug_timestamp_naming_no_timeseries_row_is_an_error(mini):
    """The dispensation clause of that agreement. With no DRUG column
    the timestamp is the only join between the two files,
...
def test_over_cap_drugs_are_ignored(mini):
    """SLOT = -1 marks a drug the slot cap dropped. The
...
# --- the cohort-wide contract ----------------------

def test_row_order_and_count_are_labels_csv_s(mini):
    """labels.csv's row order is canonical and the fold
...
def test_index_times_come_from_stays_in_labels_order(one_patient):
    """labels.csv has no INDEX_TIME column, so the value is
...
def test_no_static_data_file_when_static_feats_is_empty(one_patient):
    """STATIC_FEATS is empty for this study, so the array would
...
def test_text_strings_are_interned_once_in_canonical_order(mini):
    """Extraction assigns the row indices into text_embeddings.npy,
    first appearance in canonical row order winning index 0. embed.py
    embeds the
...
def test_blank_text_gets_no_csr_entry_and_indicator_zero(one_patient):
    """The empty string must not reach the table."""
    assert run(one_patient) == 0
...
def test_event_indicator_follows_presence_not_truth(mini):
    """process_event_data never consults the declared type, so a literal
    0 would mark the timestep as an admission. The upstream writer
...
def test_a_short_episode_is_reported_not_dropped(mini, capsys):
    """The fold arrays are positions in labels.csv, so extraction
...
def test_standardize_saves_statistics_without_scaling_the_values(
    ...
    """Extraction runs once for the cohort while
    standardization is per fold, so a single array cannot carry one
    fold's scaling. __getitem__ applies it at load time."""
...
def test_fold_statistics_are_written_per_fold(one_patient):
    """load_dataset reads summary_statistics_fold{i}.npz."""
...
def test_contract_fails_on_a_config_name_with_no_entry():
    """How AGE, SCU and ADMIT_DAD were missed."""
...
def test_contract_fails_when_size_and_category_map_disagree():
    """This used to raise mid-extraction, on real data,
    hours in."""
    config = {'VALUED_FEATS': ['A'], 'EVENT_FEATS': [], 'TEXT_FEATS': [],
              'DRUG_FEATS': [], 'STATIC_FEATS': []}
...
# --- re-running into a used directory ------------

def test_a_rerun_clears_artifacts_of_a_removed_feature(one_patient):
    """Regression. On a re-run with ``DRUG_FEATS: []`` the previous
    run's drug arrays used to survive beside a metadata.pkl that no
    longer mentions them, and the webapp finds drugs by filename."""
    assert run(one_patient) == 0
...
# --- fold rows must address this cohort ------------------

def test_fold_rows_past_the_end_of_the_cohort_are_refused(one_patient):
    """The fold arrays are positions in labels.csv. A stale fold
